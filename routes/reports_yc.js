...
router.get('/webPage',  async function (ctx, next) {
		...
		else if (patientid != "" && visitid != ""){
			sql = `SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,c.orderid AS visitid,b.reqid,b.conclusion,b.bioxreport FROM patient a, report b, orderlist c WHERE a.reqid = b.reqid AND a.patientid = ? AND c.orderid = ?`;
	        value = [patientid, visitid];	
		}
	    else
	    {
...
router.get('/findPage',  async function (ctx, next) {
	...
	else if (patientid!="")
	{
		...
		JOIN 
			report b 
			ON a.reqid COLLATE utf8mb4_general_ci = b.reqid COLLATE utf8mb4_general_ci
		LEFT JOIN 
			patientinfo_yc c 
			ON a.reqid COLLATE utf8mb4_general_ci = c.reqid COLLATE utf8mb4_general_ci
		LEFT JOIN 
			orderlist d  
			ON a.reqid COLLATE utf8mb4_general_ci = d.reqid COLLATE utf8mb4_general_ci
		WHERE 
			a.patientid = ?;
		`;
	value=[patientid];
	...
	{
		sql=`SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,b.reqid,b.conclusion,b.bioxreport FROM patient a,report b where a.reqid = b.reqid and a.inpatientid= ?`;
		value=[inpatientid];
		findType="2";
	}
	else if (outpatientid!="")
	{
		sql=`SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,b.reqid,b.conclusion,b.bioxreport FROM patient a,report b where a.reqid = b.reqid and a.outpatientid= ?`;
		value=[outpatientid];
		findType="3";
	}
	else if (inpatientid != "" && orderid != "")
	{
		sql = `SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,c.orderid,b.reqid,b.conclusion,b.bioxreport FROM patient a,report b ,orderlist c WHERE a.reqid = b.reqid AND a.inpatientid = ? AND c.orderid = ?`;
    	value = [inpatientid, orderid];
    	findType = "4";
	}	
	else if (outpatientid != "" && orderid != ""){
		sql = `SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,c.orderid,b.reqid,b.conclusion,b.bioxreport FROM patient a,report b ,orderlist c WHERE a.reqid = b.reqid AND a.outpatientid = ? AND c.orderid = ?`;
    	value = [outpatientid, orderid];
    	findType = "5";
	}
	else if (patientid != "" && orderid != ""){
		sql = `SELECT a.patientid,a.name,a.sex,a.age,a.inpatientid,a.outpatientid,c.orderid,b.reqid,b.conclusion,b.bioxreport FROM patient a,report b ,orderlist c WHERE a.reqid = b.reqid AND a.patientid = ? AND c.orderid = ?`;
    	value = [patientid, orderid];
    	findType = "6";
	}
	else
	{
